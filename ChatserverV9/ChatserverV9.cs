...
class ClientHandler
{
    ...
    public async Task HandleAsync()
    {
        try
        {
            ...
            if (helloMsg == null || helloMsg.Type != "join" || helloMsg.Nickname == null)
            {
                await writer.WriteLineAsync("[시스템] 닉네임을 입력하세요.");
                return;
            }
            ...
            // 딕셔너리에 등록
            lock (ChatServerV9.lockObj)
            {
                if (!ChatServerV9.Rooms.ContainsKey(roomName))
                {
                    ChatServerV9.Rooms[roomName] = new List<TcpClient>();
                }

                ChatServerV9.Rooms[roomName].Add(client);
                ChatServerV9.clientNames[client] = nickname;
                ChatServerV9.clientRooms[client] = roomName;
            }
        ...
        finally
        {
            lock (ChatServerV9.lockObj)
            {
                ChatServerV9.Rooms[roomName].Remove(client);
                ChatServerV9.clientNames.Remove(client);
                ChatServerV9.clientRooms.Remove(client);
            }
            ...
            case "/users":
                {
                    ...
                    lock (ChatServerV9.lockObj)
                    {
                        foreach (var c in ChatServerV9.Rooms[roomName])
                        {
                            if (ChatServerV9.clientNames.TryGetValue(c, out var name))
                            {
                                users.Add(name);
                            }
                        }
            ...
            default:
                {
                    if (msg.Command!.StartsWith("/w "))
                    {
                        ...
                        lock (ChatServerV9.lockObj)
                        {
                            foreach (var kvp in ChatServerV9.clientNames)
                            {
                    ...
                    else if (msg.Command!.StartsWith("/create "))
                    {
                        ...
                        lock (ChatServerV9.lockObj)
                        {
                            isExist = ChatServerV9.Rooms.ContainsKey(newRoom);
                        }
                        ...
                        ChatServerV9.Rooms[newRoom] = new List<TcpClient>();
                    }
                    else if (msg.Command!.StartsWith("/join "))
                    {
                        ...
                        lock (ChatServerV9.lockObj)
                        {
                            isExist = ChatServerV9.Rooms.ContainsKey(targetRoom);
                        }
    ...
    private async Task MoveToRoom(string newRoom)
    {
        ...
        lock (ChatServerV9.lockObj)
        {
            ChatServerV9.Rooms[newRoom].Add(client);
            ChatServerV9.clientRooms[client] = newRoom;
            roomName = newRoom;
        }
    ...
    private async Task LeaveRoom()
    {
        lock (ChatServerV9.lockObj)
        {
            if (!string.IsNullOrEmpty(roomName) && ChatServerV9.Rooms.ContainsKey(roomName))
            {
                ChatServerV9.Rooms[roomName].Remove(client);

                if (ChatServerV9.Rooms[roomName].Count == 0)
                {
                    ChatServerV9.Rooms.Remove(roomName);
                }
                ChatServerV9.clientRooms.Remove(client);
            }
        }
    ...
    private async Task PrintRoomList()
    {
        List<string> roomNames;
        lock (ChatServerV9.lockObj)
        {
            roomNames = new List<string>(ChatServerV9.Rooms.Keys);
        }
